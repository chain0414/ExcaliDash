...
}); const sanitizeSvgUseTags = (content: string): string => content.replace(/<use\b[^>]*\/\s*>|<use\b[^>]*>(?:\s*<\/use\s*>)?/gi, (useTag) => { const hrefMatch = useTag.match(/\shref\s*=\s*"([^"]*)"/i) ?? useTag.match(/\shref\s*=\s*'([^']*)'/i) ?? useTag.match(/\sxlink:href\s*=\s*"([^"]*)"/i) ?? useTag.match(/\sxlink:href\s*=\s*'([^']*)'/i); const hrefValue = hrefMatch?.[1]?.trim(); if (!hrefValue || !safeLocalUseHrefPattern.test(hrefValue)) { return ""; } const withoutXlinkHref = useTag.replace(
/\sxlink:href\s*=\s*(?:"[^"]*"|'[^']*')/gi, "" ); if (/\shref\s*=/i.test(withoutXlinkHref)) { return withoutXlinkHref.replace(
/\shref\s*=\s*(?:"[^"]*"|'[^']*')/i,
          ` href="${hrefValue}"`
); }
      return withoutXlinkHref.replace(/<use\b/i, `<use href="${hrefValue}"`);
}); const sanitized = purify
.sanitize(svgContent, { ALLOWED_TAGS: [ "svg", "defs", "pattern", "g", "image", "use", "rect", "circle", "ellipse", "line", "polyline", "polygon", "path", "text", "tspan", ], ALLOWED_ATTR: [ "xmlns", "xmlns:xlink", "version", "id", "viewBox", "preserveAspectRatio", "x", "y", "width", "height", "cx", "cy", "r", "rx", "ry", "x1", "y1", "x2", "y2", "points", "d", "fill", "fill-opacity", "fill-rule", "clip-rule", "stroke", "stroke-width", "stroke-opacity", "stroke-linecap", "stroke-linejoin", "stroke-miterlimit", "stroke-dasharray", "stroke-dashoffset", "opacity", "transform", "vector-effect", "patternUnits", "patternContentUnits", "font-size", "font-family", "font-weight", "letter-spacing", "text-anchor", "dominant-baseline", "href", "xlink:href", ], FORBID_TAGS: [ "script", "foreignObject", "iframe", "object", "embed", "style", "link", "symbol", "marker", "clipPath", "mask", "filter", ], FORBID_ATTR: [
"onload", "onclick", "onerror", "onmouseover", "onfocus", "onblur", "src", "action", "style", "class", ], KEEP_CONTENT: true, })
.trim(); return sanitizeSvgUseTags(sanitizeSvgImageTags(sanitized)).trim(); }; export const sanitizeText = ( input: unknown, maxLength: number = 1000 ): string => { if (typeof input !== "string") return ""; const cleaned = input.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, ""); const truncated = cleaned.slice(0, maxLength); return purify
